...
message_throughput_test() ->
    ...
    ServerPid = init("message_throughput"),
    ParentPid = self(),
    ...
    Fjoin = fun (I) ->
        try
            ...
            genserver:start(ClientAtom, client:initial_state(Nick, list_to_atom(GUIName), ServerPid), fun client:handle/2),
    ...
    putStrLn("spawning ~p channels with ~p clients each", [?CONC_3_CHANS, ?CONC_3_USERS]),
